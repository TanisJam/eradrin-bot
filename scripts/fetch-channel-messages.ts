...
async function saveMessageAsMarkdown(message: MessageData, threads: Map<string, MessageData[]>) {
  ...
  logger.info(`Mensaje guardado como ${filePath}`);
}
...
async function fetchAllChannelMessages(channel: TextChannel): Promise<MessageData[]> {
  logger.info(`Obteniendo mensajes del canal #${channel.name}...`);
  ...
  while (true) {
    ...
    logger.info(`Obtenidos ${messages.size} mensajes. Total: ${allMessages.length}`);
  }
...
async function fetchAllThreadsWithMessages(channel: TextChannel): Promise<Map<string, MessageData[]>> {
  logger.info(`Obteniendo hilos del canal #${channel.name}...`);
  ...
  logger.info(`Total de hilos encontrados: ${allThreads.length}`);
...
async function fetchThreadMessagesSimple(thread: ThreadChannel): Promise<MessageData[]> {
  logger.info(`Obteniendo mensajes del hilo #${thread.name}...`);
  ...
  while (true) {
    ...
    lastId = messages.last()?.id;
    logger.info(`Obtenidos ${messages.size} mensajes del hilo. Total: ${allMessages.length}`);
  }
...
async function main() {
  try {
    logger.info('Iniciando script para obtener mensajes...');
    
    if (!CHANNEL_ID) {
      logger.error('Error: CHANNEL_ID no está configurado. Configúralo en el archivo .env o directamente en el script.');
      process.exit(1);
    }
    ...
    client.once('ready', async () => {
      ...
      try {
        ...
        if (!channel || !(channel instanceof TextChannel)) {
          logger.error('Canal no encontrado o no es un canal de texto');
          process.exit(1);
        }
        ...
        logger.info('Guardando mensajes como archivos markdown...');
...
main().catch(err => logger.error('Error fatal:', err)); 