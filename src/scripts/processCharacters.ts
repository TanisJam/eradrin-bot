...
async function processCharacter(character: any): Promise<void> {
  try {
    console.log(`Procesando: ${character.name} (${character.url})`);
    
    const response = await axios.get(character.url);
    const data = response.data;
    
    if (!data || !data.printable_hash) {
      console.warn(`No se encontraron datos para: ${character.name}`);
      return;
    }
    ...
    console.error(`Error procesando ${character.name}:`, error);
  }
}
...
async function main() {
  try {
    ...
    console.log(`Encontrados ${charactersData.length} personajes para procesar`);
